...
class StepProtocol extends MqttClient {
  ...
  listen() {
    ...
    this.on("loadDevice", this.onLoadDevice.bind(this));
  ...
  setupPanel() {
    ...
    this.content.node.style.overflowX = "auto";
  ...
  onDeviceSet(payload) {
    // console.log("device set", JSON.parse(payload));
    return;
  }
  ...
  renderToolbar() {
    ...
    const loadDevice = new ToolbarButton({
      className: "fa fa-upload",
      tooltip: "Upload device.svg",
      style: {height: "auto"},
      onClick: () => {this.trigger("loadDevice", null)}});
    loadDevice.node.style.height = "auto";

    this.toolbar.insertItem(0, 'previous', prev);
    this.toolbar.insertItem(1, 'next', next);
    this.toolbar.insertItem(2, 'save', save);
    this.toolbar.insertItem(3, 'loadDevice', loadDevice);
  }
  render() {
    if (!this.ready) {
